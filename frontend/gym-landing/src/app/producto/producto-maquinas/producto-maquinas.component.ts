...
@Component({
  selector: 'app-producto-maquinas',
  standalone: true,
  ...
  templateUrl: './producto-maquinas.component.html',
  styleUrl: './producto-maquinas.component.scss'
})
export class ProductoMaquinasComponent {
  value = '';
  max = 5000000;
  min = 0;
  step = 50000;
    ...
    this.gService.list('get-imgs-names/')
      ...
      .subscribe((data:any)=>{
        ...
        this.datosComb = this.datos
          .filter(product => product.pcategory === "Maquinas")
          .map(product => {
